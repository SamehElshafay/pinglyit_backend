...
class PaymobGatewayTest extends TestCase
{
    ...
    public function test_it_is_not_configured_until_all_three_keys_are_set(): void
    {
        ...
        PlatformSetting::set('paymob_public_key', 'pk_test_public');
        PlatformSetting::set('paymob_secret_key', 'sk_test_secret');
        $this->assertFalse($gateway->isConfigured()); // hmac still missing
    ...
    public function test_create_topup_session_sends_the_amount_in_cents_and_returns_the_checkout_url(): void
    {
        $this->configure();
        $company = Company::factory()->create();
        $company->wallet()->create(['balance' => 0]);
        ...
        $url = app(PaymobGateway::class)->createTopupSession($company, 25.5, 'egp');
        ...
        Http::assertSent(function ($request) use ($company) {
            return $request->url() === 'https://accept.paymob.com/v1/intention/'
                ...
                && $request['amount'] === 2550 // 25.50 EGP in piastres/cents
    ...
    public function test_webhook_with_a_valid_signature_credits_the_wallet(): void
    {
        $this->configure();
        $company = Company::factory()->create();
        $company->wallet()->create(['balance' => 0]);
        ...
        $obj['extras'] = ['company_id' => $company->id]; // not part of the HMAC — see class docblock
        ...
        $this->assertDatabaseHas('wallet_topups', ['provider' => 'paymob', 'provider_reference' => '998877', 'status' => 'completed']);
    }
    ...
    public function test_webhook_credit_is_idempotent_on_the_transaction_id(): void
    {
        ...
        ['obj' => $obj, 'hmac' => $hmac] = $this->signedTransaction();
        $obj['extras'] = ['company_id' => $company->id];
